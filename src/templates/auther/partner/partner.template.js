...
const build = certificate => {
  const html = `
    <div style="font-family:'Roboto','Helvetica','Arial',sans-serif; text-align:center; background:url(${laurel}) center center no-repeat; background-size:contain;">
      <h1 style="font-size:300%; font-weight:bold; text-shadow:3px 3px #e0e0e0; margin:0; padding-top:3em;">
        ${certificate.badge.name}
      </h1>
      <h2 style="font-size:200%; text-transform:uppercase; margin:0;">
        ${certificate.recipientProfile.name}
      </h2>
      <p style="font-size:125%; margin:0; margin-top: 10px;">
        ${certificate.badge.description}
      </p>
      <p style="font-size:100%; font-style:italic; margin:0; margin-top: 10px;">
        Issued on ${format(new Date(certificate.issuedOn), 'yyyy/MM/dd')}
      </p>
      <div style="text-align:center;">
        <img src="${certificate.badge.image}" alt="${certificate.badge.name}" style="max-height:150px; width:auto; margin:0; margin-top:10px;" />
      </div>
